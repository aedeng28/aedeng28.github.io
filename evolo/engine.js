// --- VERLET PHYSICS, CAMERA SCROLLER & CANVAS GRAPHICS ENGINE ---
...
document.getElementById('tb').addEventListener('click',()=>{
    if(window.started)return alert("Please reload page to change brain mode layout!");
...
document.getElementById('lb').addEventListener('click',()=>{window.started=true;try{window.pop[window.cIdx].r=window.rehydrate(JSON.parse(document.getElementById('li').value.trim()));window.pop[window.cIdx].f=0;window.cc=new Creature(window.pop[window.cIdx]);window.ticks=0;}catch(e){alert("Error parsing code!");}});
...
function nextGen(){
    window.pop.sort((a,b)=>b.f-a.f);if(window.pop.f>window.best)window.best=window.pop.f;document.getElementById("b").innerText=Math.round(window.best);
    let nP=[];let s1=new window.DNA();s1.r=window.clone(window.pop.r);s1.f=window.pop.f;let s2=new window.DNA();s2.r=window.clone(window.pop.r);s2.f=window.pop.f;nP.push(s1,s2);
